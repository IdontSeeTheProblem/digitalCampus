...
const bldgs = {
    rst: createBldg({ 
        l:5, w:5, h:5, color: 0xffff00, opacity: 1, x:0, y:2.5, z:-100, rotation: 0
    }),
    mainBldg: createBldg({ 
  ...
  object.addEventListener("click", (event) => {
    ...
    switch (name){
        case 'mainBldg':
            ...
            break;
        case 'lib':
            camera.position.set(bldg.position.x-25, bldg.position.y+15, bldg.position.z-45)
            break;
        case 'lab':
            camera.position.set(bldg.position.x+40, bldg.position.y+10, bldg.position.z-55)
            break;
        case 'mensa':
            camera.position.set(bldg.position.x+50, bldg.position.y+15, bldg.position.z+30)
            break;
        case 'dorm':
            camera.position.set(bldg.position.x+25, bldg.position.y+20, bldg.position.z+55)
            break;
    }
    controls.update()
  });
...
function navigateToBuilding({name}) {
    for (const [bldg, object] of Object.entries(bldgs)) {
        if (bldg === name) {
          ...
          switch (name){
            case 'rst':
                camera.position.set(object.position.x+95, object.position.y+60, object.position.z+95)
                break;
            case 'mainBldg':
                camera.position.set(object.position.x+10, object.position.y+20, object.position.z+50)
                break;
            case 'lib':
                camera.position.set(object.position.x-25, object.position.y+15, object.position.z-45)
                break;
            case 'lab':
                camera.position.set(object.position.x+40, object.position.y+10, object.position.z-55)
                break;
            case 'mensa':
                camera.position.set(object.position.x+50, object.position.y+15, object.position.z+30)
                break;
            case 'dorm':
                camera.position.set(object.position.x+25, object.position.y+20, object.position.z+55)
                break;
        }
...
// experminetal to limit camera movement under the floor
/*
